...
/**
 * @author Sebastian
 * Does NOT implement the ranking system
 * Only matches EXACT questions
 */
public class PlaceholderKnowledgeEngine implements KnowledgeEngine {
	...
	public AnswerResponse getAnswerToQuestion(String question) {
		...
		if(step == null)
			...
			result = new AnswerResponse(STATUS.OK, new Answer(step.stepId, step.stepDescription, step.getNeighborhoodString()));
	...
	private Step getBottom(Step step){
		if(step.downStep == null)
			return step;
		else return getBottom(step.downStep);
	}
	...
	class Step{
		...
		public String getNeighborhoodString(){
			int neighbors = 0;
			if (upStep!= null) neighbors+=1000;
			if (nextStep!= null) neighbors+=0100;
			if (downStep!= null) neighbors+=0010;
			if (previousStep!= null) neighbors+=0001;
